    ...
    <div className="flex items-start justify-between gap-3 py-2 border-b last:border-b-0">
      ...
      <div className="flex-1 min-w-0">
        {link ? (
          ...
          <div className={`break-all ${mono ? "font-mono text-xs" : "text-sm"}`}>{v ?? "-"}</div>
        )}
      </div>
      {copy ? (
        <button
...
function buildDomInjectionVM({ detail, summary, parsedPayload, ruleOneLine }) {
  ...
  return {
    ...
    kpis: [
      ...
      kpiBool("cross_site", "Cross-site", crossSite, "외부 스크립트/도메인 기원"),
    ...
    recommendations: [
      ...
      { when: crossSite === true, text: "cross-site 삽입: 외부 스크립트가 DOM을 변경했습니다(차단/allowlist 검토)." },
...
function buildUiHijackVM({ detail, summary, parsedPayload, ruleOneLine }) {
  ...
  const coveredRatio = src.coveredRatio ?? src.coverageRatio ?? src.coverRatio ?? null; // 0~1 or %
...
export default function AdminEventDetailPage() {
  ...
  const eventId =
    params.eventId ||
    sp.get("eventId") ||
    "170f1c9c0a8364b6ad7941fac4b56c2f";
  ...
  const [tab, setTab] = useState("summary"); // summary | context | activity | attribution | evidence
    ...
    <TitleCard title="이벤트 상세" topMargin="mt-2">
      ...
      {!eventId ? (
        <div className="p-4 border rounded-xl">
          ...
          <div className="text-sm opacity-70">
            예: <span className="font-mono">/app/admin_front/detail/{"{eventId}"}</span>{" "}
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 text-xs opacity-70 font-mono">
                    ruleId: {effectiveRuleId} / type: {summary.type || parsedPayload?.type || "-"} / category:{" "}
                    {vm.category}
                  </div>
                </div>
              ...
              <Section title="사건 요약">
                <div className="mt-2 text-sm space-y-2">
                  <div>
                    ...
                    <span className="font-mono break-all">{ruleDescription?.oneLine || "-"}</span>
                  </div>
                  <div>
                    <span className="opacity-60">이벤트 유형(type):</span>{" "}
                    <span className="font-mono break-all">{summary.type || parsedPayload?.type || "-"}</span>
                  </div>
                  {summary.pageHost ? (
                    <div>
                      <span className="opacity-60">위험 페이지 도메인:</span>{" "}
                      <span className="font-mono break-all">{summary.pageHost}</span>
                    </div>
                  ) : null}
                </div>
              </Section>
          ...
          {tab === "context" ? (
            <Section title="Context">
              ...
              <KV k="page" v={summary.page || "-"} link={summary.page || ""} copy={summary.page || ""} />
              <KV k="targetHost/targetOrigin" v={summary.targetHost || "-"} mono />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  {vm.activityRows.map((row) => (
                    <div key={row.label} className="p-3 rounded-xl border">
                      ...
                      <div className="font-mono break-all">{row.value ?? "-"}</div>
                    </div>
                  ))}
                </div>
              ) : (
          ...
          {tab === "evidence" ? (
            <Section title="Evidence">
              <KV
                ...
                mono
                copy={detail?.payload?.payloadHash || detail?.payloadHash || ""}
              />
              <KV
                k="payloadTruncated"

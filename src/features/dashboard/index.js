  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-2 mt-0 grid-cols-1 gap-6">
        {severityData?.labels ? (
          <BarChart title="위험도별 분포" chartData={severityData} />
        ) : (
          <div>차트 로딩 중...</div>
        )}

        {domainData?.labels ? (
          <BarChart title="도메인별 트랜드" chartData={domainData} />
        ) : (
          <div>차트 로딩 중...</div>
        )}
      </div>
        ...
        <div className="grid lg:grid-cols-2 mt-10 grid-cols-1 gap-6">
          <div className="overflow-x-auto w-full">
            <table className="table w-full">
              ...
              <tbody>
                {highSeverityData.map((l, k) => {
                  return (
                    <tr key={k}>
                      <td>
                        <div className="flex items-center space-x-3">
                          <div className="avatar">{l.eventId}</div>
                          <div>
                            <div className="font-bold">{l.severity}</div>
                          </div>
                        </div>
                      </td>
                      <td>{l.scoreDelta}</td>

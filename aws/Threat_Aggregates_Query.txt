...
// Keep shard selection consistent with ingest (sha256 first 8 hex -> mod)
const aggShardFor = (s) =>
  parseInt(
...
async function fanoutTrendBetween({ pkBase, skFrom, skTo, shardKey }) {
  // For a specific key (ruleId/domain) we can hit exactly one shard, matching ingest's aggShardFor().
  if (shardKey !== undefined && shardKey !== null && String(shardKey).length > 0) {
    const shard = aggShardFor(shardKey);
    return await queryShardPkSkBetween({ pk: pkBase, shard, skFrom, skTo });
  }

  // Fallback: fan-out (useful if shardKey is unknown or for debugging)
  const all = [];
...
function parseTrendSk(sk) {
  ...
  const m = /^DAY#(\d{4}-\d{2}-\d{2})#SEV#(.+)$/.exec(String(sk || ""));
  return m ? { day: m[1], sev: m[2] } : null;
}

function foldTrendItemsToDays({ items, startDay, endDay }) {
  const acc = new Map(); // day -> {sev->cnt}
  const sevSet = new Set();
  for (const it of items) {
    const p = parseTrendSk(it.sk);
    if (!p) continue;
    const m = acc.get(p.day) || new Map();
    const c = Number(it.cnt || 0);
    m.set(p.sev, (m.get(p.sev) || 0) + c);
    acc.set(p.day, m);
    sevSet.add(p.sev);
  }

  const severities = Array.from(sevSet).sort((a, b) => String(a).localeCompare(String(b)));
  const out = [];
  let cur = startDay;
  while (cur <= endDay) {
    const m = acc.get(cur) || new Map();
    const buckets = {};
    for (const sev of severities) buckets[sev] = m.get(sev) || 0;
    out.push({ day: cur, buckets });
    cur = addDaysIso(cur, 1);
  }
  return { severities, days: out };
}

function parseRangeOrDefault(qs, fallbackDays = 30) {
  const rStart = normDayFlexible(qs.startDay);
  const rEnd = normDayFlexible(qs.endDay);
  const { start, end } = (rStart && rEnd) ? { start: rStart, end: rEnd } : lastNDaysRange(fallbackDays);
...
export const handler = async (event) => {
  try {
    ...
    if (path.endsWith("/aggregates/topn/domains-range")) return await handleTopnDomains(event);
    if (path.endsWith("/aggregates/topn/rules-range")) return await handleTopnRules(event);
    if (path.endsWith("/aggregates/severity-range")) return await handleSeverity(event);
